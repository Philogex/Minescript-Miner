...
class NativeExactGeometryTest(unittest.TestCase):
    def test_exact_projective_kernel(self):
        ...
        with tempfile.TemporaryDirectory(
            prefix="minescript-exact-geometry-test-"
        ) as temp_dir:
            executable = Path(temp_dir) / "exact_geometry_test"
            subprocess.run(
                [
                    compiler,
                    "-std=c++17",
                    ...
                    "-I",
                    str(boost_include),
                    str(project_root / "native/tests/exact_geometry_test.cpp"),
